...
BEGIN
    ...
    CREATE MASTER KEY
    ENCRYPTION BY PASSWORD = 'r3P1iKa5x_123';
...
(
    ...
    WHERE name = 'blocking_monitor'
)
BEGIN

    DROP EVENT SESSION blocking_monitor
    ON DATABASE;
...
(
    ...
    WHERE name =
    'https://$STORAGE_ACCOUNT_NAME.blob.core.windows.net/$XEVENT_CONTAINER_NAME'
)
BEGIN
...
CREATE EVENT SESSION blocking_monitor
ON DATABASE
...
(
    SET filename =
    'https://$STORAGE_ACCOUNT_NAME.blob.core.windows.net/$XEVENT_CONTAINER_NAME/blocking_monitor.xel',
        max_file_size = 100,
...
ALTER EVENT SESSION blocking_monitor
ON DATABASE
STATE = START;
"
...
echo -e "${GREEN}==========================================${NC}"
echo -e "${GREEN}Extended Events Session Created For Demo DB.${NC}"
echo -e "${GREEN}==========================================${NC}"
