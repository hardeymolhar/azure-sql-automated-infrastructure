...
| Terraform | Infrastructure as Code (automated, repeatable deployment) |
| Key Vault | Customer-managed encryption keys, secure key storage, and key rotation |
| Private Endpoints | Private-only access (no public endpoints), Network isolation (VNet-restricted access) |
| Python    | Workload simulation (data ingestion & query execution), DTU/performance observation |
